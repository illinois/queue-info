...
- There's now a button to delete all questions on a queue when the queue is closed. If you reach the end of office hours and there are still a lot of people on the queue, this makes it easy to clear the remaining questions so that you can start with an empty queue the next day. Thanks to [Rittika](https://github.com/rittikaadhikari) for implementing this!
